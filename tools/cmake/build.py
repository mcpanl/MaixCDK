...
def rebuild(build_path, configs, toolchain_info, verbose):
    ...
    # cmake: source dir is always project path (build may be nested under build/<plat>_<arch>)
    ...
    maix_arch = configs.get("MAIX_ARCH", "") or kconfigs.get("MAIX_ARCH", "")
    with open(path, "w", encoding="utf-8") as f:
        ...
        if maix_arch:
            code += "#define MAIX_ARCH \"{}\"\n".format(maix_arch)
            if maix_arch == "riscv64":
                code += "#define CONFIG_ARCH_RISCV64 1\n"
                code += "#define MAIX_ARCH_IS_RISCV64 1\n"
            elif maix_arch == "arm64":
                code += "#define CONFIG_ARCH_ARM64 1\n"
                code += "#define MAIX_ARCH_IS_ARM64 1\n"
        code += "#endif\n"
        f.write(code)
...
def distclean(project_path, build_path, dist_path):
    if os.path.exists(build_path):
        # Prefer cmake clean if build tree is configured
        ...
        if os.path.exists(project_cmakelist):
            # Only remove auto-generated project CMakeLists if present
            try:
                with open(project_cmakelist, "r", encoding="utf-8") as f:
                    head = f.read(200)
                if "auto generated by MaixCDK" in head or "don't modify it manually" in head:
