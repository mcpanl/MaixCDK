				      ...
				      zonhor_camera_graph_profile_t *profile)
{
	...
	/*
	 * Group0-Ch0: rotate 90° → logical (H,W), buffer width = align_up(H,64).
	 * GDC channel attr uses pre-rotation size (W x align_up(H,64)).
	 */
	z_rotate_extent(sw, sh, Z_ROTATION_90, &rot_ext);
	profile->rotated_main = rot_ext;

	/* Display: logical 172x320, buffer 192x320.
	 * Valid content is centered (10px pad each side) to match HW letterbox. */
	z_frame_layout_calc(ZONHOR_DISP_LOGICAL_W, ZONHOR_DISP_LOGICAL_H, &disp_ext);
	{
		uint32_t pad_x = (disp_ext.buffer_width - disp_ext.logical_width) / 2u;
		z_apply_padding(&disp_ext, pad_x, 0,
				ZONHOR_DISP_LOGICAL_W, ZONHOR_DISP_LOGICAL_H);
	}
	...
	fill_chn(&profile->group0.channels[0], "rotated_main_yuv",
		 0, 0, Z_PIXEL_YUV_NV21, &rot_ext, false, true, Z_ROTATION_90);
	...
	fill_chn(&profile->group1.channels[0], "user_mem_pipeline",
		 1, 0, Z_PIXEL_YUV_NV21, &sensor_in, true, false, Z_ROTATION_0);
	...
	fill_chn(&profile->group2.channels[0], "display_preview",
		 2, 0, Z_PIXEL_RGB888, &disp_ext, false, true, Z_ROTATION_0);
	fill_chn(&profile->group2.channels[1], "main_venc_input",
		 2, 1, Z_PIXEL_RGB888, &main_ext, false, true, Z_ROTATION_0);
	fill_chn(&profile->group2.channels[2], "sub_pipeline_input",
		 2, 2, Z_PIXEL_YUV_NV21, &half_ext, false, true, Z_ROTATION_0);
	...
	fill_chn(&profile->group3.channels[0], "sub_venc_input",
		 3, 0, Z_PIXEL_YUV_NV21, &half_ext, true, false, Z_ROTATION_0);
	fill_chn(&profile->group3.channels[1], "sub_npu_input",
		 3, 1, Z_PIXEL_RGB888, &half_ext, true, false, Z_ROTATION_0);
	{
		...
		fill_chn(&profile->group3.channels[2], "sub_reserved",
			 3, 2, Z_PIXEL_YUV_NV21, &reserved, true, false, Z_ROTATION_0);
	}
...
void zonhor_dump_camera_graph_profile(const zonhor_camera_graph_profile_t *profile)
{
	...
	for (g = 0; g < 4; ++g) {
		...
		for (c = 0; c < grp->channel_count; ++c) {
			...
			printf("  ch%u %s fmt=%u rot=%d create_only=%d enabled=%d\n",
