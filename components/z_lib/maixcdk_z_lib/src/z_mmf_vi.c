...
CVI_S32 Z_VI_INIT(Z_VI_CTX_S *pstViCtx)
{
    ...
    astVpssChnAttr[VpssChn].u32Width = Z_WIDTH;
    astVpssChnAttr[VpssChn].u32Height = Z_HEIGHT;
...

    {
        ...
        for (_chn = 0; _chn < VPSS_MAX_PHY_CHN_NUM; _chn++)
            CVI_VPSS_DisableChn(VpssGrp, _chn);
        CVI_VPSS_StopGrp(VpssGrp);
        CVI_VPSS_DestroyGrp(VpssGrp);
        SAMPLE_PRT("[Z_VI_INIT] pre-cleanup Group %d done\n", VpssGrp);
    }
    ...
    stVpssGrpAttr1.u32MaxW = Z_WIDTH;
    stVpssGrpAttr1.u32MaxH = Z_HEIGHT;
    stVpssGrpAttr1.u8VpssDev = 0;

    astVpssChnAttr1[VpssChn1].u32Width = Z_WIDTH;
    astVpssChnAttr1[VpssChn1].u32Height = Z_HEIGHT;
    ...
    {
        VPSS_CHN _chn;
        SAMPLE_COMM_VPSS_UnBind_VO(VpssGrp1, VPSS_CHN0, 0, 0);
        for (_chn = 0; _chn < VPSS_MAX_PHY_CHN_NUM; _chn++)
            CVI_VPSS_DisableChn(VpssGrp1, _chn);
    ...
    RECT_S stDefDispRect = {0, 0, Z_HEIGHT, Z_WIDTH};
    SIZE_S stDefImageSize = {Z_HEIGHT, Z_WIDTH};
    VO_CHN VoChn = 0;
