...
namespace z::nn
{
    ...
    err::Err MUD::load(const std::string &model_path)
    {
        ...
        {
            printf(">>> z_nn.cpp MUD::load begin maixcam_load_cvimodel \n");
    ...
    NN::NN(const std::string &model_path, bool dual_buff)
    {
        ...
        if (!model_path.empty())
        {
            printf(">>> z_nn.cpp load() \n");

            err::Err e = load(model_path);
            printf(">>> z_nn.cpp load() done \n");
    ...
    err::Err NN::load(const std::string &model_path)
    {
        ...
        printf(">>> z_nn.cpp load() impl begin _mud.load()\n");
        err::Err e = _mud.load(model_path);
        printf(">>> z_nn.cpp load() impl begin _mud.load() done\n");
        if (e != err::ERR_NONE)
        {
            return e;
        }
