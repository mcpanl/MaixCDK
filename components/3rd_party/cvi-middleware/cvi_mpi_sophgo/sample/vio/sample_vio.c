...
#include <errno.h>
#include <unistd.h>
#include <fcntl.h>      // open, O_RDONLY, O_RDWR, O_NONBLOCK
...
CVI_S32 SAMPLE_VIO_TWO_DEV_VO(void)
{
	...
	stSizeOut.u32Width  = 640;
	stSizeOut.u32Height = 480;
	...
	stVoConfig.stVoPubAttr.enIntfSync  = VO_OUTPUT_480x640_60;
	...
	do {
		...
		//use ./sample_vio & then step3
		//or step1./sample_vio 6  step2,ctrl + z
		//3,order
		//echo 0   > /tmp/vo_cmd   # change chn 0
		//echo 1   > /tmp/vo_cmd   # change chn 1
		//echo 255 > /tmp/vo_cmd   # exit
		//4,fg
