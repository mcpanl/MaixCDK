...
template <typename T_OBJ>
int Maix_GUI<T_OBJ>::init(display::Display *screen, touchscreen::TouchScreen *touchScreen_)
{
    /*Initialize LVGL*/
    lvgl_init(screen, touchScreen_);

    this->screen = screen;
    this->touchscreen = touchScreen_;
...
Activity_LVGL(Maix_GUI_Activity* activity, lv_obj_t *parent, lv_obj_t *scr = NULL,
    ...
    void destroy(Maix_Activity_MSG *msg = nullptr)
    {
        if(activated)
        {
            ...
            {
                lv_ui_mutex_lock(-1);
                lv_screen_load_anim(parent_scr, LV_SCR_LOAD_ANIM_MOVE_RIGHT, 200, 0, true);
                lv_ui_mutex_unlock();
            }
            else
            {
