...
static const char *MODEL_PATH = "/root/models/yolov5s.mud";
...
int _main(int argc, char* argv[])
{
    ...
    nn::YOLOv5 yolo("", false);
    err::Err e = yolo.load(MODEL_PATH);
    err::check_raise(e, "load yolov5 model failed");
    ...
    while (!app::need_exit())
    {
        ...
        draw_header(*img, "NN YOLO");

        std::vector<nn::Object> *result = yolo.detect(*img);
        if (result)
        {
            for (auto &obj : *result)
            {
                ...
                std::string label;
                if (obj.class_id >= 0 && (size_t)obj.class_id < yolo.labels.size())
                    label = yolo.labels[obj.class_id];
                else
                    label = std::string("class_") + std::to_string(obj.class_id);

                char buf[96] = {0};
                snprintf(buf, sizeof(buf), "%s %.2f", label.c_str(), obj.score);
