...
int _main2(int argc, char* argv[])
{
    ...
    auto *disp = new display::Display(552, 368);
    ...
    while(!app::need_exit()) {
        ...
        img = new image::Image(552, 368);
        img->draw_string(28, 28, "Hello, world!" + to_string(time::ticks_ms()), image::COLOR_RED, 3, 2);
    ...
    img = new image::Image(552, 368);
    img->draw_string(28, 28, "Hello, world!", image::COLOR_RED, 3, 2);
    Bytes *img_bytes = img->to_bytes();
...
//    log::info("disp: %p", disp);
    ...
    while(!app::need_exit())
    {
        ...
        z_lib_vpss_take_frame(&stFrameInfo, 1000);
//        z_lib_vi_take_frame(&stFrameInfo, 1000);

        printf("Got frame: %ux%u\n", stFrameInfo.stVFrame.u32Width, stFrameInfo.stVFrame.u32Height);

        z_lib_vo_push_frame(&stFrameInfo);

//        z_lib_vi_release_frame(&stFrameInfo);
        z_lib_vpss_release_frame(&stFrameInfo);
