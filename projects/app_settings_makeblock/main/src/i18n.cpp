...
/*
font generate in https://lvgl.io/tools/fontconverter

1. zh font: https://github.com/adobe-fonts/source-han-sans/releases/download/2.004R/SourceHanSansCN.zip
...
/*
2. fonts: https://lvgl.io/assets/others/FontAwesome5-Solid+Brands+Regular.woff
...
static const std::map<string, string> locale_zh_dict = {
    {"Confirm", "确认"},
    {"Language", "语言"},
    {"OK", "好的"},
    ...
    {"About", "关于"},
    {"My Device", "我的设备"},
    {"Install Runtime", "安装运行库"},
    {"Upgrade", "更新"},
    {"Upgrading", "更新中"},
    {"Install", "安装"},
    {"Upgrade MaixPy", "更新 MaixPy"},
    {"Power", "电源"},
    {"Scan", "扫描"},
    {"Connect", "连接"},
    {"Connecting...", "连接中..."},
    ...
    {"Cancel", "取消"},
    {"Current", "当前"},
    {"Set success, please wait for got IP, if long time no IP, check password", "设置成功, 等待获取 IP, 如果长时间没有 IP, 请检查密码"},
    {"QR shared by phone", "手机分享的二维码"},
    {"or visit", "或访问"},
    {"Scan QR", "扫描二维码"},
    {"Version", "版本"},
    {"System Version", "系统版本"},
    ...
    {"Latest", "最新"},
    {"Latest version", "最新版本"},
    {"Runtime Version", "运行库版本"},
    {"version", "版本"},
    {"loading", "加载中"},
    {"Strongly recommend directly reflashing the system image!", "强烈建议直接重新烧录系统!"},
    {"Download from:", "下载自:"},
    {"Not install yet", "未安装"},
    {"USB Settings", "USB 设置"},
    {"USB mode", "USB 模式"},
    {"NIC mode", "网卡模式"},
    {"rndis for Windows, ncm for others", "Windows 用 rndis, 其它用 ncm"},
    {"Date", "日期"},
    ...
    {"Please visit", "请访问"},
    {"Warning", "警告"},
    {"Please visit documentation", "请访问文档"},
    {"Need reboot after turn on again!", "再次打开后必须重启"},
    {"Volume", "音量"},
    {"Speaker", "扬声器"},
    {"Microphone", "麦克风"},
    {"Privacy and Usage", "隐私协议"}
};

static const std::map<string, string> locale_en_dict = {
    {"wifi_hint", "1. Generate QR code from:\n   1. maixhub.com/wifi\n   2. WiFi setting of mobile phone\n2. Scan QR code\n3. Connect WiFi"},
    {"℃", " Centigrade"}
};

static const std::map<string, const std::map<string, string>> locales_dict = {
    {"zh", locale_zh_dict},
    {"en", locale_en_dict}
};
