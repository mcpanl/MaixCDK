...
void on_language_gui(Maix_GUI_Activity *activity, void *obj, Maix_Activity_MSG *msg, void *args)
{
    ...
    for (size_t i = 0; i < i18n::locales.size(); i++)
    {
        if (i18n::locales[i] == locale)
        {
    ...
    lv_obj_add_event_cb(btn, [](lv_event_t *e) {
        ...
        if (code == LV_EVENT_CLICKED)
        {
            ...
            uint16_t idx = lv_dropdown_get_selected(ddlist);
            log::info("set language: %s, %s\n", i18n::locales[idx].c_str(), i18n::names[idx].c_str());
            // save to config file
            app::set_sys_config_kv("language", "locale", i18n::locales[idx]);
            // change locale immediately.
            trans.set_locale(i18n::locales[idx]);
            // show toast
            static char buff[12];
...
void on_language_gui_destroy(Maix_GUI_Activity *activity, void *obj, Maix_Activity_MSG *msg, void *args)
{
}
